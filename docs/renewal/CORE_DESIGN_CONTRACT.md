...
**레거시 경로 deprecate 계획(문서화)**

> 기준일: **2025-12-18**

* **신규 표준 경로**: 신규 개발/신규 프론트는 반드시 `/api/{channel}/v1/...`만 사용
  * 예: `/api/fdk/v1/chat`, `/api/web/v1/chat`
* **레거시 유지 범위**: `/api/chat`, `/api/chat/stream`은 하위호환을 위해 유지하되,
  * 신규 기능 추가는 하지 않고(기능 동결), **버그/보안/운영 이슈만 최소한으로 수정**
* **멀티테넌트 분리 원칙**: 멀티테넌트 전용 경로는 `/api/multitenant/*`로 분리하고,
  레거시 `/api/chat`은 “tenant 헤더가 있으면 멀티테넌트, 없으면 레거시”로 점진 전환
* **제거(삭제) 시점**: 실제 제거는 별도 PR로 진행하며,
  * 사전 공지(문서/릴리즈 노트)와 영향 범위 점검(사용처/트래픽)을 먼저 끝낸 뒤에 결정
